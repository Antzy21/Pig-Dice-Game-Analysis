...
At this point we had a rough idea of every equation in any simultaneous set. For clarity, we ordered them as follows in the probabilities matrix, $X$.
=======
...
Now we have a rough idea of every equation in any simultaneous set. For clarity, we will order them as follows in the probabilities matrix, $X$.
>>>>>>> 2ec93797b356c919ff64953bdff8f99c49e1b802
\begin{align}
	\begin{bmatrix}
		P_{i,j,0}\\
		P_{i,j,1}\\
...
=======
The first problem to our assumptions so far is that we will need all of these equations. This only outlines all of the ones that could be needed. Take for example, the set built around $0,0$, where A is banking on $20$. $P_{0,0,50}$ for example would never be considered, as the strategy would have banked many rolls ago. For banking on 20 strategy, the game state $0,0,50$ and many others would be impossible. This idea of impossible game states was touched on in the introduction, the difference here is that this one is specific to a strategy. As discussed in the introduction, a turn score of 1 is also not possible for pig.

This problem has a surprisingly elegant solution. The definition of an impossible game state is one that cannot be reached from other game states. The coefficient's matrix will be filled from top to bottom, meaning lower turn score probabilities are completed first. So if we get to any probability that is an impossible game state, it would not have been called by any other game states before it. Therefore all the coefficient values for it would be zero. This is a simple to check in our program. We then go on to ignore this row of the matrix, which is the same as it not being there. With this problem solved, we can use the set probability matrix described above, without worrying if the probability is relevant or not.
>>>>>>> 2ec93797b356c919ff64953bdff8f99c49e1b802
...
<<<<<<< HEAD
We then ran through every game state backwards, collecting the probabilities as we solved them. Remember that matrices might need the probabilities calculated from higher game states, so whilst we built our collection of probabilities, we will had to call upon them as we worked down. Once we had completed every matrix calculation, we had a generalised working code that could calculate the probabilities of winning at any game state, for any two strategies, for any given dice probability and to any winning score.
=======
<<<<<<< HEAD
...
>>>>>>> 52a1555d5abba7c28c28b8143944e32166960b09
>>>>>>> 2ec93797b356c919ff64953bdff8f99c49e1b802
...
<<<<<<< HEAD
As a brief explanation of how it works, the code takes two important inputs (not including a general 'to win' score and dice probability, which is also implemented). The main input is the strategy that we wish to optimise against, let us call this strategy B. The other input is another strategy that we will be modifying, to continually get better against B, let us call this strategy A. The reason we wanted to be able to start from any point was to see if there were multiple optimals against B, local maximas. Strategies A and B can be in any format as we will mad use of "strategies to matrices" to get them in our preferred matrix form. The code then runs through the simultaneous sets again, modifying A to be the best it possibly can against B.
=======
As a brief explanation of how it works, the code takes two important inputs (not including a general 'to win' score and dice probability, which is also implemented). The main input is the strategy that we wish to optimise against, let us call this strategy B. The other input is another strategy that we will be modifying, to continually get better against B, let us call this strategy A. The reason we wanted to be able to start from any point was to see if there were multiple optimals against B, local maximas. Strategies A and B can be in any format as we will make use of "strategies to matrices" to get them in our preferred matrix form. The code then runs through the simultaneous sets again, modifying A to be the best it possibly can be against B.
>>>>>>> 2ec93797b356c919ff64953bdff8f99c49e1b802

As we did for calculating the probabilities, we will built matrices, this time a little differently. In the probability matrices we were looking up what the strategy does and building the matrices around what the choice was. However, this time we wanted to consider both rolling and banking at each game state. We could then see which out of rolling and banking had the highest probability of winning. If the strategy's choice is not one with the highest chance of it winning, we could then change the choice the strategy makes at that game state and also the probability it has.

The method behind this is a little more complicated as all the probabilities are interlinked. By changing one probability, it will change all the others, potentially making rolling or banking at another game state a better choice. This meant that we may not have had to change strategy A multiple times before it stabablises at each set.
...
We used our probability calculator to test it against other strategies. It beats all strategies that we had produced. We then tested it against Nella's optimal, to which both it and Nella's optimal has the same probability of winning when each going first. From previous testing, we had learnt that if these probabilities were the same, it was highly likely that the strategies were the same. We could test to see if the strategies were the same by simply comparing them in matlab with the command \textbf{matlab code here}. This confirmed this idea, as every element in our strategy was the same as Nellas.

When Optimising against Nella's strategy, the strategy produced is an exact copy of Nella's. This is the same as how repeated implications of the optimal finder eventually converges and doesn't change, meaning that it reached Nella's optimal.
