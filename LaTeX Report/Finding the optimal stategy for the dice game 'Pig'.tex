...
A game state is defined by the current position of the game, player A's banked score, player B's banked score and the turn score of whichever player's turn it is. These two banked scores can vary between zero and 100. The turnscore can accumilate from 0 up until the game is won, so when the player has 0 banked points, they could obtain a turn score of 100 before winning. The mistake would be to assume that this means there are $100\times100\times100 = 1$ Million game states. The reason this is not true is because many are not possible. Take a banked score of $90$ and a turn score of $50$ for example, this game would have ended a long time ago when the player passed a turn score of $10$ winning them the game! We called this an impossible game state, because it is not possible to reach this state, playing by the rules of the game. Another type of impossible game state is with a turn score of $1$. The smallest turn score larger than $0$ that a player can have is by rolling a $2$, giving them a turn score of $2$.

Let us now define a strategy as an collection of choices for every possible game state, a choice to roll or bank. This lets us define a large amount of possible strategies. However there could have been more variables to consider, such as how many times the player has rolled so far. From every strategy that is possible from our definition, our next question, and the main focus of this paper, was what the best strategy is to play Pig. What strategy is most likely to win against any other, and be crowned the Optimal Strategy for the dice game Pig.

At any game state it is possible to produce linear equations to determine if it is better to roll or bank by looking at the probability of winning from that game state. Suppose there are two strategies A and B. Consider that it is A's turn and they are in position ($i,j,k$) where $i$ is A's banked points, $j$ is B's banked points, and $k$ is A's sum total of points so far on their turn. Let $P_{ijk}$ denote the probability of A winning from that position. We let $Q_{jik}$ denote probability of B winning from the equivalent position on their turn. Note that the position of $j$ and $i$ have swapped because it is from the perspective of each player. We are able to produce the following equations describing these probabilities based on the choice the strategies make,
\begin{equation}\label{1.2.1.a}
P^{Roll}_{ijk} = \dfrac{1}{6} (1-Q_{ij0}) + \dfrac{1}{6}\sum^{6}_{r=2}P_{ijk+r}
\end{equation}
if A rolls and
\begin{equation}\label{1.2.1.b}
P^{Bank}_{ijk} = 1-Q_{jik}
\end{equation}
if A banks their points.
\subsection{Neller's work}
...
The first step we took to familiarise ourselves with Pig was to do some tests of the game. We had a MATLAB code to run a simulation of the game. It took strategies as inputs and gave us the result of which strategy won. We began by testing a few basic strategies, such as ``Bank at $20$''. We then tried to formulate our own strategies in an attempt to beat the aforementioned strategies. We were very successful in beating these strategies, which led us to believe that there are many strategies that in theory would be optimal but in practice are easily beaten.
...
For the coding, using Github was very useful for collaberation. Github allowed us all to work in the same repository and edit files similtaneously. Instead of sharing files that could easily go out of date, missing important details and other issues, we all know that we had the latest version of our files at any point that we were working on them. Github also showed us every edit that each member in our group made with comments to explain the changes. This way we could all understand what changes had been made. This LaTeX docment for example was edited on Github. These benefits Github gave us sped up every task we had regarding code.
...
Firstly, we looked at the probability A has of winning when it banks, denoted $P^{Bank}_{i,j,k}$. A passes to B's turn by banking. So $P^{Bank}_{i,j,k}$ will be equal to the probability of A winning on B's turn. The probability $Q_{j,i,k}$ is the probability that player B has of winning on their turn at their game state $j,i,k$. As long as the game is finite, Pig will always have a winner. Therefore, the sum of the probabilities from both players at any point in the game will be $1$. Note, that this is from any point in the game, not at any game state, as a game state is specific to a player. From this we concluded that when it is player B's turn at game state $j,i,k$, player A will have a probability of winning that is $1-Q_{j,i,k}$. By banking, A adds their turn score to their banked score, and B's turn starts with a turn score of zero. Thus, we define the probability A has at any game state of banking as
...
After writing this out, we see the other probabilities it relies on. Next, we want to see what the strategies do at these game states and then write out the probability equations for them. In this example, let A roll at $97,99,2$ and B roll at $99,97,0$
...
This method of calculation works well. However, before we can do the same for every set of equations, we must to look at the interesting case of banking. For example, instead of rolling at $97,99,2$, let A bank. The equations are now:
...
We do not need to write out the equation for $Q_{99,99,0}$ because we already calculated it's value to be $\frac{6}{7}$ in our first example. We then solve the equations, getting a much different result:
...
From this we conclude that when a strategy banks, it relies on knowledge of the probabilities of game states with higher banked scores than itself, which shall be called ``higher game states'' from this point onwards. Note that it will not rely on game states ``lower'' than it as the game will always be progressing forward and a player cannot lose banked points. Once we have solved the sets containing higher game states first, we can work down the rest of the sets and solve them all successfully.

Solving these equations would mean we can calculate every probability of winning from every game state for any given strategy. From this we can check $P_{0,0,0}$, which is the probability that any strategy has of winning the game. Computational power was required to give us the probability values that we wanted to calculate. This is where we turned to MATLAB for assistance.
...
In our introduction, we defined a strategy as a collection of choices. Another way, and the way that we were introduced to strategies, is by a MATLAB function. These functions take inputs of ``myscore'',``yourscore'', and ``turnscore'', then output a ``choice'' that is a logical value of $1$ or $0$. Below is an example of Bank on 20.
