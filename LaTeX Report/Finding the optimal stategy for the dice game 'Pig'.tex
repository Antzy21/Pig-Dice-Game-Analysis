...
For Pig, Matlab completes all 1 million sets in 5 seconds. Although we have every probability on Strategy A's turn $P_{i,j,k}$ and every probability on Strategy B's turn $Q_{i,j,k}$, the main outputs that we are interested in, are $P_{0,0,0}$ and $Q_{0,0,0}$. These are the probabilities that each strategy has of winning, when going first.
...
>>>>>>> 193296285d6111bee468d3b247148de40bd738d3
...
\section{Conclusions}
\subsection{Our Optimal findings}
At the start of our project we wanted to find the optimal strategy. We defined it as a strategy that has the highest probability of winning when played against all other strategies. Nella's strategy, was the best fit to this definition. For every other strategy, we were able to find an optimal against it that beat it, whereas Nella was not beaten by any strategy (that we could find from our code). We were happy with this result as a lot of our testing reinforced and confirmed this.
